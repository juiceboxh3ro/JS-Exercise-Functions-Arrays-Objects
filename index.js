...
function getLastCarInfo(arr) {
  let lastCar = arr.length - 1;
  return `This is a ` + arr[lastCar].car_make + arr[lastCar].car_model;
}
...
function getCarInfoById(arr, id) {
  ...
  return `This is a ` + arr[carID].car_make + arr[carID].car_model;
}
...
function sortCarInventory(arr) {
  ...
  return arr.sort((a, b) => a.car_model.toUpperCase() > b.car_model.toUpperCase() ? 1 : -1);
}
